...
"""
import numpy as np
from tsp_w_hopfield import Hopfield
        ...
        for col in range(v.shape[1]):
            for row in range(v.shape[0]):
                if v[row][col] == 1:
                    ...
                    prev_row = row
                    break
        print("From City {} To City {}".format(prev_row + 1, first_row + 1))
        dist += d[prev_row][first_row]
        summation += dist
        mini = min(mini, dist)
        maxi = max(maxi, dist)
        print("Distance:", dist, "\n")

    print("\nMin: {}\nMax: {}\nAverage: {}".format(mini, maxi, summation / 3.0))
