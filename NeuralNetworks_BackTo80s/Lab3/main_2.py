...
def calc_eucledean_distance(data_point: List[float], weight_vector: np.ndarray[float]):
    return math.sqrt(
        sum(
            [(value - weight) ** 2 for value, weight in zip(data_point, weight_vector)]
        )
    )



def find_closest_vector(data_point: List[float], W: np.ndarray[np.ndarray[float]]):
