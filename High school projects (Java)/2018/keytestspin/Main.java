...
public void keyPressed(KeyEvent e) {
    if (e.getKeyCode ()==39) {
       if (renderPanel.getOperations() % renderPanel.getChange () ==0)
       {
          renderPanel.subtractHeading();
       }
       renderPanel.subtractOperation();
...
class MyJPanel extends JPanel implements MouseMotionListener,MouseListener {
    ...
    int nablaX;
    int nablaY;
    int changeAngleAfterNOperations;
    int nOperations;
    int startXMotion=0;
    int startYMotion=0;
    int finXMotion=0;
...
public MyJPanel (int headingSlider, int pitchSlider,int rollSlider,int FoVSlider,int deltaX,int deltaY) {
    //конструктор, ничего необычного
     ...
     this.changeAngleAfterNOperations=nablaX/2;
     this.nOperations=0;
...
new Vertex(-100, -100, -100, 1),
    ...
    //управление полями класса из MyFrame
     ...
     public int getChange () {
		  return changeAngleAfterNOperations;
		 }
