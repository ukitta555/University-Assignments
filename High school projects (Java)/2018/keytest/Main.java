    ...
    public MyFrame (String s) { //переделанный под ООП вариант нашего окна
      ...
      pane.setLayout(new BorderLayout());
        ...
        double height = screenSize.getHeight();
         renderPanel = new MyJPanel(headingSlider,pitchSlider,rollSlider,FoVSlider,(int)(width/2)); // передаем все слайдеры чтобы Джава не ругалась на статик
...
class MyJPanel extends JPanel implements MouseMotionListener,MouseListener {
    ...
    int finXMotion=0;
    int finYMotion=0;
public MyJPanel (int headingSlider, int pitchSlider,int rollSlider,int FoVSlider,int deltaX) {
...
new Vertex(-100, -100, -100, 1),
    ...
    //управление полями класса из MyFrame
     ...
     //преопределние методов для расчета сдвига курсора    
     public void mouseDragged(MouseEvent e)  {
	    System.out.print (e.getLocationOnScreen().getX()+" "+e.getLocationOnScreen().getY()+" ");
